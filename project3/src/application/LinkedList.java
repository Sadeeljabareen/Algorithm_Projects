...
public class LinkedList {
	...
	public void addFirst(Edges edge) { // O(1)
	...
	public void addLast(Edges vertix) { // O(n)
	...
	public Edges get(String name) { // O(n)
	...
	public LinkedListNode getNode(String name) {
		if (first != null) { // checking for empty list
			...
			while (curr != null) {
				if (curr.getEdge().getDestination().getCapital().getName().equals(name))
					return curr; // found
	...
	public Edges getFirst() { // O(1)
		if (first == null)
			return null;
		return first.getEdge();
	}

	public Edges getLast() { // O(n)
		if (first == null)
			return null;
