...
public class Tree implements Comparable<Tree> {
	...
	private Tree left = null; // Left child of the node
	private Tree right = null; // Right child of the node
	private static int numberOfLeafNode; // Counter for the number of leaf nodes

	// Constructors
	public Tree(byte Data, int frequency) {
	...
	private String traverse(Tree curr) {
		if (curr != null)
	...
	public String toString() {
		...
		if (ByteContent != null)
			return "0" + ButtonAction.byteToBinaryString(ByteContent);
		else
			return "1"; // For non-leaf nodes, return "1"
	}

	// Method to assign binary codes to each byte in the tree
	public static void gaveCodeForEachByte(Tree root) {
		StringBuilder okBuilder = new StringBuilder();
		getPath(root, okBuilder);
	}

	private static void getPath(Tree root, StringBuilder code) {
		if (root == null)
			return;
	...
	public void getLeafNodes(Tree[] array) {
		numberOfLeafNode = 0;
		addLeafNodes(this, array);
		Arrays.sort(array);
	}

	private void addLeafNodes(Tree node, Tree[] array) {
		if (node == null) {
